...
public class Trick implements Serializable {
	...
	public void play(Player p, Card c) throws IllegalMove {
		...
		// Check if its players actual turn
		if (getNextToPlay() != p.direction) throw new IllegalMove("Not your turn");

		// Check if the card is even in the players hand
		...
		for (Card card : p.getHand()) {
			if (card != null) {
				...
				// Check if it follows suit
				if (leadCard != null) {
					if (leadCard.suit().ordinal() == card.suit().ordinal()) {
						suitInHand = true;
					}
				}
			}
		}
		if (!cardInHand) throw new IllegalMove("You tried to play a card not in your hand");

		// Check if you have the suit in hand but the card the player placed was not following the suit
		// of the lead card placed
		if (leadCard != null) {
			if (leadCard.suit().ordinal() != c.suit().ordinal() && suitInHand) {
				throw new IllegalMove("You cannot play that card because you have the lead suit in your hand");
			}
		}
